 ...
 * Enables hot-reloadable, extensible plugin architecture for SENTINAL
 */
...
export class PluginLoader extends EventEmitter {
     ...
     */
    async loadPlugin(pluginRef) {
        let pluginPath = null;
        let pluginName = pluginRef;

        try {
            pluginPath = this.resolvePluginPath(pluginRef);
            pluginName = path.basename(pluginPath, path.extname(pluginPath));

            if (!fs.existsSync(pluginPath)) {
                console.warn(`[PLUGIN] Plugin not found: ${pluginRef}`);
                return false;
            }
    ...
    resolvePluginPath(pluginRef) {
        if (!pluginRef) {
            throw new Error('Plugin reference is required');
        }

        if (path.isAbsolute(pluginRef) || pluginRef.includes(path.sep)) {
            return path.resolve(pluginRef);
        }

        const candidates = [
            path.join(this.pluginDir, pluginRef),
            path.join(this.pluginDir, `${pluginRef}.js`),
            path.join(this.pluginDir, `${pluginRef}.mjs`)
        ];

        const match = candidates.find(candidate => fs.existsSync(candidate));
        return match || candidates[1];
    }
    ...
    async findPluginForIntent(intent, userInput) {
        ...
        if (lowerInput === "what can you do" || lowerInput === "help" || lowerInput === "capabilities") {
            return "core-capabilities-plugin";
        }
        if (lowerInput.includes("upgrade abilities") || lowerInput.includes("upgrade your abilities")) {
            return "self-evolution-plugin";
        }
