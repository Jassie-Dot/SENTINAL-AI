...
export class SocketManager {
    constructor(httpServer, contextManager, pluginLoader, aiHandler, intentRecognizer, autonomyEngine, systemTools) {
        ...
        this.policy = new PolicyEngine({
            approvalTtlMs: Number(process.env.SENTINAL_APPROVAL_TTL_MS || 2 * 60_000),
        });
    ...
    handleConnection(socket) {
        ...
        socket.on("policy:approve", (data) => this.handleApproval(socket, data));
        socket.on("chat:message", (data) => {
            const msg = String(data?.message || '').trim();
            // Allow "approve <token>" as a simple UX.
            const m = /^approve\s+([A-Za-z0-9_-]{6,})$/i.exec(msg);
            if (m) {
                this.handleApproval(socket, { token: m[1] });
                return;
            }
    ...
    handleApproval(socket, data) {
        const sessionId = socket.id;
        const token = String(data?.token || '').trim();
        if (!token) {
            socket.emit("chat:response", { message: "[SYSTEM] Missing approval token.", source: "policy" });
            return;
        }

        const approval = this.approvals.consume(token, { sessionId });
        if (!approval) {
            socket.emit("chat:response", { message: "[SYSTEM] Approval token invalid or expired.", source: "policy" });
            return;
        }

        socket.emit("chat:response", {
            message: `[SYSTEM] Approval accepted for ${approval.category}. Re-run the command now.`,
            source: "policy",
            data: { category: approval.category }
        });
    }
    ...
    async handleAIGeneration(socket, sessionId, userInput, intentAnalysis) {
        ...
        while ((match = toolCodeRegex.exec(finalResponse)) !== null) {
            ...
            try {
                ...
                if (toolName) {
                    // Policy check BEFORE executing anything
                    const evalResult = this.policy.evaluate({ toolName, toolInput });
                    if (evalResult?.blocked) {
                        socket.emit("chat:response", {
                            message: `[SYSTEM] Action blocked (${evalResult.category}): ${evalResult.reason}`,
                            source: "policy"
                        });
                        continue;
                    }
                    if (evalResult?.requiresApproval) {
                        const approval = this.policy.mintApproval({
                            sessionId,
                        ...
                        socket.emit("chat:response", {
                            message:
                                `[SYSTEM] Approval required for ${approval.category}.\n` +
                                `To proceed, type: approve ${approval.id}\n` +
                                `(expires in ${Math.round((approval.expiresAt - approval.createdAt) / 1000)}s)`,
                            source: "policy",
                            data: { approval: { id: approval.id, category: approval.category, expiresAt: approval.expiresAt } }
                        });
                        continue;
                    }
